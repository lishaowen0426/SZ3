#ifndef SZ3_BFS_BLOCKWISE_ITERATOR_HPP
#define SZ3_BFS_BLOCKWISE_ITERATOR_HPP
...
class bfs_block_data : public std::enable_shared_from_this<bfs_block_data<T, N>> {
   public:
    /**
    ...
    class block_iterator {
       public:
         ...
         */
        block_iterator(std::shared_ptr<bfs_block_data> &&mddata_, size_t block_size_) noexcept
            : mddata(mddata_), block_sizes{block_size_}, current_block(0) {
            std::fill(offset.begin(), offset.end(), 0);
        }

        block_iterator(std::shared_ptr<bfs_block_data> &&mddata_, std::vector<size_t> block_sizes_)
            : mddata(mddata_), block_sizes(std::move(block_sizes_)), current_block(0) {
            if (block_sizes.empty()) {
                throw std::invalid_argument("block_sizes must not be empty");
            }
            if (N != 1 && block_sizes.size() != 1) {
                throw std::invalid_argument("variable block sizes are only supported for 1D data");
            }
            std::fill(offset.begin(), offset.end(), 0);
        }
        ...
        ALWAYS_INLINE bool next() {
            if (N == 1 && block_sizes.size() > 1) {
                offset[0] += block_sizes[current_block];
                current_block++;
                return (current_block < block_sizes.size() && offset[0] < mddata->dims[0]);
            }

            size_t i = N - 1;
            offset[i] += block_sizes[0];
        ...
        ALWAYS_INLINE std::array<std::pair<size_t, size_t>, N> get_block_range() const {
            ...
            const size_t current_block_size = block_sizes[(N == 1 && block_sizes.size() > 1) ? current_block : 0];
            for (uint i = 0; i < N; i++) {
                range[i].first = offset[i];
                range[i].second = std::min(offset[i] + current_block_size, mddata->dims[i]);
            }
            return range;
        }
        ...
        friend bfs_block_data;
        std::shared_ptr<bfs_block_data> mddata;
    ...
    bfs_block_data(T *data_, const std::vector<size_t> &dims_, size_t padding_ = 0, bool data_valid = true)
        : padding(padding_) {
        if (dims_.size() != N) {
    ...
    block_iterator block_iter(std::vector<size_t> block_sizes) {
        return block_iterator(this->shared_from_this(), std::move(block_sizes));
    }
...
ALWAYS_INLINE void bfs_foreach (T *data, size_t offset, const std::array<size_t, N> &begins,
